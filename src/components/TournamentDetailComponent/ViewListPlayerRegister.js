...
export default function ViewListPlayerRegister(props) {
  const { teamInTournament, setViewList } = props;
  ...
  const getAllPlayerByTeamId = () => {
    ...
    response.then(res => {
       getAllPlayerInTournamentByTeamInTournamentId(res.data.playerInTeamsFull);
    }).catch(err => {
      setLoading(false);
      console.error(err);
    })
  ...
  const getAllPlayerInTournamentByTeamInTournamentId = async (data) => {
    ...
    const response = await getAllPlayerInTournamentByTeamInTournamentIdAPI(teamInTournament.id);
    if(response.status === 200){
      for(const item of response.data.playerInTournaments){
        const findNewPlayer = data.find(itemData => itemData.id === item.playerInTeamId);
        ...
        <div class="modal-content">
          ...
          <div
                    style={{
                      width: "73vw",
                      height: 350,
                      overflowY: playerRegister != null && playerRegister.length > 5 ? "scroll" : "hidden",
                    }}
                  >
                    <table
                      ...
                      <tbody className="listFootballPlayerChoice">
                                ...
                                >
                                  ...
                                  <td><img style={{
                                    ...
                                    height:50,
                                    marginLeft:10
                                  }} src={item.footballPlayer.playerAvatar} /></td>
