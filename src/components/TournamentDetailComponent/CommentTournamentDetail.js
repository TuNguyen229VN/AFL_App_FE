  ...
  const getComment = async (created) => {
    try {
      ...
      const response = await axios.get(
        `https://afootballleague.ddns.net/api/v1/Comment?tounamentID=${idTour}&orderType=${sort}&pageIndex=1&limit=${numOfContent}`
      );
  ...
  const createComment = async () => {
    try {
      ...
      const response = await axios.post(
        `https://afootballleague.ddns.net/api/v1/Comment?tournamentID=${idTour}`,
        data,
        {
          headers: {
