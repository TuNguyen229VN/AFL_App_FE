...
function ReportTeamDetail() {
  ...
  const getResult = async () => {
    try {
      ...
      const response = await axios.get(
        `https://afootballleague.ddns.net/api/v1/tournament-results?teamId=${idTeam}&page-offset=1&limit=5`
      );
  ...
  const getStatistic = async () => {
    try {
      const response = await axios.get(
        `https://afootballleague.ddns.net/api/v1/TeamInMatch/Result?teamId=${idTeam}`
      );
        ...
        <div className="match">
          ...
          <div className="match__list">
            <div className="match__item">
              ...
              <p className="match__number">
                {statistic && statistic.totalMatch}
              </p>
            </div>
            <div className="match__item">
            ...
            {result.length > 0 &&
              result.map((item, index) => (
                <tr>
                  ...
                  <td>
                    <p>
                      {item.totalWinMatch}-{item.totalDrawMatch}-
                      {item.totalLoseMatch}
                    </p>
                  </td>
