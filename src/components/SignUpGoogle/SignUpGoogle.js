...
function SignUpGoogle(props) {
	...
	const [inputValues, setInputValues] = useState({
		lastName: '', firstName: props.userInfo.displayName,phone:"",gender:''
	      });
		...
		const response = await axios.post('https://afootballleague.ddns.net/api/v1/users/CreateWithGoogle', data,{
			headers: {
				"Content-type": "multipart/form-data"
			      }
		});
		console.log(response.data);
		const login = await axios.post('https://afootballleague.ddns.net/api/v1/auth/login-with-google',{
			"tokenId" : `${props.token}`
		      },{
			headers: {
...
return (
	...
	<form action="" method="POST" className="signupgg__form" onSubmit={(e) =>{
		       postNewAccount();
		      e.preventDefault();
	      }}>
